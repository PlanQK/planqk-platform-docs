...
This API description gives potential users of the service a manual on how they are able to communicate with the serivce, namely how they can send the input for problems and receive the solutions.  
...
The user code template already includes `openapi-spec.yml`, which is a generic API description and can be adapted for your own service.
This mainly refers to defining the types of input and output for the corresponding REST-methods which will be described below.
...
#### Service API: `POST`
Any method related to the actual execution of the service is listed within the Service API section of the API description. The POST method is used to send input for the service execution. Using this method gives gives back an execution id, which is required for receiving results of the execution.

#### Service API: `GET` (id)
The id generated by the POST method can be used for this GET method to check on the status of the service execution.
The possible status values are quite self-explanatory but just to make sure:  

If the exeuction is still running, you should get `"status": "RUNNING"` or `"status": "PENDING"` (especially in the beginning ).  
...
When the exeuction finished successfully, you should see `"status": "SUCCEEDED"`. 
#### Service API: `GET` (result)
If the service execution either failed or succeeded, you can get the results (or details on the occured problems) via this mehtod.
